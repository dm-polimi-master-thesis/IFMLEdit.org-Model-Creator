...
function update(cell) {
    ...
    return {
        ...
        resultsFields: tree['page-list'] ? _.map(tree['page-list'].attributes.fields, function (f) {return { label: f.label, value: f.label, type: ko.observable(f.type || 'text'), name: ko.observable(f.name || '') }}) : [],
        dataEntryFields: tree['data-entry-form'] ? _.map(tree['data-entry-form'].attributes.fields, function (f) {return { label: f.label, value: f.label, type: ko.observable(f.type || 'text'), name: ko.observable(f.name || '') }}) : _.map(tree['page-details'].attributes.fields, function (f) {return { label: f.label, value: f.label, type: ko.observable(f.type || 'text'), name: ko.observable(f.name || '') }})
    }
}
